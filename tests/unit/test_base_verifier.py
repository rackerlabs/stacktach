...
class BaseVerifierTestCase(unittest.TestCase):
    def setUp(self):
        self.mox = mox.Mox()
    ...
    def test_run_once_notifications(self):
        self._mock_exchange_create_and_connect()
        self.verifier_with_notifications._run_once(callback=mox.Not(mox.Is(None)))
        self.mox.ReplayAll()
        self.verifier_with_notifications.run_once()
        self.mox.VerifyAll()

    def test_run_once_no_notifications(self):
        self.mox.StubOutWithMock(self.verifier_without_notifications, '_run_once')
        self.verifier_without_notifications._run_once()
        self.mox.ReplayAll()
        self.verifier_without_notifications.run_once()
        self.mox.VerifyAll()
